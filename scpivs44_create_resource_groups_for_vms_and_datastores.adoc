...
+
Das SnapCenter Plug-in für VMware vSphere führt keine Kompatibilitätsprüfungen für alle ONTAP Versionen durch, sondern nur für ONTAP Versionen 8.2.1 und früher. Aktuelle Informationen zum SnapCenter-Support finden Sie daher immer unter https://imt.netapp.com/matrix/imt.jsp?components=134348;&solution=1517&isHWU&src=IMT["NetApp Interoperabilitäts-Matrix-Tool (IMT)"^] .
