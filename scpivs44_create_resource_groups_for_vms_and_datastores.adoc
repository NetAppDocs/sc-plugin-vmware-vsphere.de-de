...
---
= Erstellen Sie Ressourcengruppen für VMs und Datenspeicher im SCV
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* Für FlexGroup Volumes in NetApp MetroCluster Umgebungen, wenn Sie ONTAP 9.8 oder 9.9 verwenden, starten Sie den SnapCenter Plug-in für VMware vSphere Dienst neu und synchronisieren Sie SnapMirror Beziehungen nach einem Switchover oder Switchback erneut, bevor Sie Ressourcengruppen sichern. In ONTAP 9.8 können Backups nach einem Switchback hängen bleiben; dieses Problem ist in ONTAP 9.9 behoben.
