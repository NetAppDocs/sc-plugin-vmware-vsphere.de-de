...
| SnapCenter vSphere Client-GUI | VM- und Datastore-Backup VMDK-Anbindung und -Trennung Datastore-Mount und unmounten Sie VM und VMDK Restore der Gastdatei und Ordner | Backups von VMs und Datenspeichern, die mit der VMware vSphere Client GUI durchgeführt wurden 
