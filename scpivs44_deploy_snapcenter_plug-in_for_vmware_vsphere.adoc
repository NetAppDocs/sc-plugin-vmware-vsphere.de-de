...
* Um VMs auf vVol-Datenspeichern zu schützen, stellen Sie zuerst ONTAP tools for VMware vSphere bereit.  Informationen zu unterstützten ONTAP Toolversionen finden Sie im https://imt.netapp.com/matrix/imt.jsp?components=134348;&solution=1517&isHWU&src=IMT["NetApp Interoperabilitäts-Matrix-Tool"^] .  ONTAP -Tools stellen Speicher auf ONTAP und dem VMware-Webclient bereit und konfigurieren ihn.
