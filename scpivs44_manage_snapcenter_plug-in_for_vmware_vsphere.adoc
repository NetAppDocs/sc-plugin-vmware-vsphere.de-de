...
+
1) SnapCenter-Plug-in für VMware vSphere anzeigen Zusammenfassung 2) SnapCenter-Plug-in für VMware vSphere-Dienste starten 3) SnapCenter-Plug-in für VMware vSphere-Dienste beenden 4) Benutzername und Passwort ändern, um sich anzumelden SnapCenter-Plug-in für VMware vSphere UI 5) MySQL-Passwort ändern 6) MySQL-Sicherung und -Wiederherstellung

** Option 2: Systemkonfiguration
+
1) virtuelle Maschine neu starten 2) virtuelle Maschine herunterfahren 3) Benutzerpasswort ändern 4) Zeitzone ändern 5) NTP-Server ändern 6) SSH-Zugriff deaktivieren 7) Jail-Festplattengröße erhöhen (/jail) 8) Upgrade 9) VMware Tools installieren

** Option 3: Netzwerkkonfiguration
+
1) IP-Adresseinstellungen anzeigen 2) IP-Adresseinstellungen ändern 3) Einstellungen für die Domänennamensuche anzeigen 4) Einstellungen für die Suche nach Domänennamen ändern 5) statische Routen anzeigen 6) statische Routen ändern 7) Änderungen festlegen 8) Ping a Host

** Option 4: Support und Diagnose
+
1) Support Bundle generieren 2) Access Diagnostic Shell 3) Remote-Zugriff aktivieren 4) Core Dump Bundle generieren
