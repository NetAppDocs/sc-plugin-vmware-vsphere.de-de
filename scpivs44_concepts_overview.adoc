...
NOTE: Verbessern Sie den VMware-Schutz mit NetApp Backup and Recovery, das 3-2-1-Backups, Ransomware-Schutz, automatisierte Backup-Verwaltung, verbesserte Skalierbarkeit, geringere Kosten und zentrale Backup-Verwaltung von Datenspeichern und VMs über mehrere vCenter Instanzen hinweg über eine einzige Oberfläche bietet. Sie können SCV-Richtlinien, Snapshots und Zeitpläne zu NetApp Backup and Recovery migrieren und den gesamten VM-Schutz über eine zentrale Konsole verwalten. Weitere Informationen finden Sie unter link:https://docs.netapp.com/us-en/data-services-backup-recovery/["NetApp Backup and Recovery Dokumentation"].

====
SnapCenter Plug-in für VMware vSphere ist eine eigenständige virtuelle Appliance (im Open Virtual Appliance-Format), die Datenschutzdienste für VMs und Datenspeicher bereitstellt und Datenschutzdienste für SnapCenter anwendungsbasierte Plug-ins unterstützt. Dieses Dokument beschreibt, wie das SnapCenter Plug-in für VMware vSphere bereitgestellt und verwendet wird und enthält eine Kurzanleitung. SnapCenter Plug-in für VMware vSphere wird als Linux-basierte virtuelle Appliance bereitgestellt.
