...
[role="lead"]
Verwenden Sie diese Aufgabe, um die in SnapCenter Plug-in for VMware vSphere konfigurierten vCenter-Anmeldedaten zu aktualisieren. Diese Anmeldedaten werden für den Zugriff auf vCenter verwendet. Wenn sich das vCenter-Passwort ändert, heben Sie die Registrierung von ONTAP tools for VMware vSphere auf und registrieren Sie ONTAP tools dann mit dem neuen Passwort erneut, damit vVol-Backups weiterhin funktionieren.
